...
static void USB_OTG_EnableCommonInt(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  (*(volatile uint32_t *)&pdev->regs . GREGS->GINTSTS = 0xFFFFFFFF);
                                ...
                                uint16_t            len)
{
  ...
  if (pdev->cfg.dma_enable == 0)
  {
    ...
    {
      (*(volatile uint32_t *)fifo = *((__packed uint32_t *)src));
    }
  }
  return status;
}
                         ...
                         uint16_t len)
{
  ...
  {
    *(__packed uint32_t *)dest = (*(volatile uint32_t *)fifo);
                               ...
                               USB_OTG_CORE_ID_TypeDef coreID)
{
  ...
  pdev->regs.GREGS = (USB_OTG_GREGS *)(baseAddress +     0x000);
  pdev->regs.DREGS =  (USB_OTG_DREGS  *)  (baseAddress +     0x800);
  
  for (i = 0; i < pdev->cfg.dev_endpoints; i++)
  {
    pdev->regs.INEP_REGS[i]  = (USB_OTG_INEPREGS *)        (baseAddress + 0x900 +         (i * 0x20));
    pdev->regs.OUTEP_REGS[i] = (USB_OTG_OUTEPREGS *)       (baseAddress + 0xB00 +         (i * 0x20));
  }
  pdev->regs.HREGS = (USB_OTG_HREGS *)(baseAddress +     0x400);
  pdev->regs.HPRT0 = (uint32_t *)(baseAddress + 0x440);
  
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    pdev->regs.HC_REGS[i] = (USB_OTG_HC_REGS *)(baseAddress +       0x500 +         (i * 0x20));
  }
  for (i = 0; i < pdev->cfg.host_channels; i++)
  {
    pdev->regs.DFIFO[i] = (uint32_t *)(baseAddress + 0x1000 +      (i * 0x1000));
  }
...
USB_OTG_STS USB_OTG_CoreInit(USB_OTG_CORE_HANDLE *pdev)
{
  ...
  else  
  {
    ...
    if(pdev->cfg.phy_itface == 3)
    {
      ...
      i2cctl.d32 = (*(volatile uint32_t *)&pdev->regs . GREGS->GI2CCTL);
      i2cctl.b.i2cdevaddr = 1;
      i2cctl.b.i2cen = 0;
      i2cctl.b.dat_se0 = 1;
      i2cctl.b.addr = 0x2D;
      (*(volatile uint32_t *)&pdev->regs . GREGS->GI2CCTL = i2cctl . d32);
