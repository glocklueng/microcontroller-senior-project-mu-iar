  ...
  case 0x00:
    ...
    case 0x06: 
      if( (req->wValue >> 8) == 0x21)
      {
        ...
        len = ((((67-9)) < (req->wLength)) ? ((67-9)) : (req->wLength));
      }
...
static uint8_t  usbd_cdc_DataIn (void *pdev, uint8_t epnum)
{
  ...
  if (USB_Tx_State == 1)
  {
    ...
    {
      if (APP_Rx_length > *(uint16_t *)(((USB_OTG_CORE_HANDLE *)pdev)->dev . pConfig_descriptor + 57)){
        USB_Tx_ptr = APP_Rx_ptr_out;
        USB_Tx_length = *(uint16_t *)(((USB_OTG_CORE_HANDLE *)pdev)->dev . pConfig_descriptor + 57);
        
        APP_Rx_ptr_out += *(uint16_t *)(((USB_OTG_CORE_HANDLE *)pdev)->dev . pConfig_descriptor + 57);
        APP_Rx_length -= *(uint16_t *)(((USB_OTG_CORE_HANDLE *)pdev)->dev . pConfig_descriptor + 57);    
      }
      else 
      {
        USB_Tx_ptr = APP_Rx_ptr_out;
